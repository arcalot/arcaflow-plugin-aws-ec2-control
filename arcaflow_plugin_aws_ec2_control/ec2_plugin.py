...
class NodeActionParams:
    ...
    action: NodeAction = field(
        metadata={"name": "action", "description": "The action to execute."}
    )
    instance_id: str = field(
        metadata={"name": "addr", "description": "The AWS instance ID."}
    )
    ...
    aws_access_private_key: str = field(
        metadata={
...
def get_boto3_ec2_client(instance_id, key_id, access_key, region):
    session = boto3.Session(
        aws_access_key_id=key_id, aws_secret_access_key=access_key, region_name=region
    )
...
@plugin.step(
    ...
    description="Runs AWS EC2 actions",
...
def aws_action(
    ...
    """
    The function runs the specified EC2 commands, and optionally waits for them to complete.
    ...
    client, resource = get_boto3_ec2_client(
        params.instance_id,
        params.aws_access_key_id,
        params.aws_access_private_key,
        params.aws_region,
    )
    if params.action.value == NodeAction.REBOOT.value:
        if (
            resource.state["Name"] == "stopped"
            or resource.state["Name"] == "terminated"
        ):
    ...
    intended_final_power_state = (
        params.action.value == NodeAction.REBOOT.value
        ...
        try:
            if params.action.value != NodeAction.TERMINATE.value:
                wait_for_power_state(
                    resource, intended_final_power_state, params.wait_timeout
                )
            else:
    ...
    if not params.wait or final_power_state_on == intended_final_power_state:
        return "success", SuccessOutput(
            int(1000 * (time.time() - start_time)), final_power_state_on
        )
    else:
        return "error", ErrorOutput(
            "Did not reach desired final state within timeout period. Power state is "
